	...
	public class IndexModel : PageModel
	{
		...
		public async Task<IActionResult> OnGetAsync(CancellationToken cancellationToken)
		{
			...
			using (var db = new AppDbContext(connectionString))
			{
				...
				for (int i = 1; i <= 13; i++)
					lst.Add(new Person { Age = 27, CreateDate = DateTimeOffset.UtcNow, FullName = $"SinjulMSBH" });
			...
			IReadOnlyList<Person> people = await context.People.OrderByDescending(c => c.Id).Take(13).ToListAsync(cancellationToken);

			if (people.Count > 0) People = people;
